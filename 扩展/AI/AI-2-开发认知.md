...
│ 查询理解 │ 检索增强 │ 生成优化 │ 评估反馈 │
├─────────────┼─────────────┼─────────────┼───────────┤
│ Query 改写 │ 混合检索 │ 提示工程 │ RAGAS 评估 │
│ 意图识别 │ 向量搜索 │ 思维链 │ 人工审核 │
│ 多轮对话 │ 全文检索 │ 多模型融合 │ A/B 测试 │
├─────────────┴─────────────┴─────────────┴───────────┤
│ 数据与基础设施层 │
│ 向量 DB 文档解析 模型服务 │
└─────────────────────────────────────────────────────┘
...
5. RAGFlow: 是一个开源、本地化部署的检索增强生成（RAG）系统
   - 中高 (需一定技术背景)
   - 开源；支持私有化部署
   - 对复杂格式文档（如 PDF、表格、扫描件）进行深度解析，实现高精度、可溯源的文档问答

- RAGFlow 的主要竞争对手在易用性（如 Dify、FastGPT）或灵活性（如 Anything-LLM、LangChain）上各有侧重。
