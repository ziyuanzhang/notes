...
5. Langflow:是一个用于构建 RAG和多代理 AI 应用的低代码应用构建器。
6. langchain: 深度定制的王者；给算法和后端工程师用的，

7. MCP: 是为单个智能体配备“工具箱”的规范

8. A2A: 多智能体系统（让多个智能体“团队协作”的协议;）
...
9. n8n: 通用型工作流自动化平台（则是搭建整个“自动化工厂”的平台）
   - 中 (可视化但逻辑复杂)
   - 开源（公平代码）；极易自托管，资源消耗低
...
* 按发展阶段分类：1. Naive RAG（初级/朴素 RAG）；2. Advanced RAG（高级 RAG）；3. Modular RAG（模块化 RAG）
* 按特定的技术流派/变体分类 ：1. GraphRAG（基于知识图谱的 RAG）；2. LightRAG；5.Agentic RAG（代理 RAG）
  1. GraphRAG： 采用模块化社区结构和分层回答策略，更适合于全局性理解和多层次的复杂问题分析；
  2. LightRAG： 则侧重于向量化图索引和增量更新机制，适用于高效、动态的数据环境。
  3. 模块化 RAG：将 RAG 的各个步骤组件化（如搜索模块、记忆模块、路由模块、预测模块等），可以根据任务动态组合

## 模型与显卡 -- 完整版（不是量化版）

1. 7B 用 32G 显卡跑（费劲）； 14B 需要 80G 显存；
2. 公司部署 deepSeek 满血版（通用型的 - 67B） 需要 200W+；
3. 公司 24G： 2-3 张，微调 7B 模型就够了（不需要满血版）；

## 提问

- Mac mini（M2 + 24GB 内存） + macOS Sequoia 15.5 + Xcode 16.4 学习 AI 开发(部署大模型)， 不影响办公（例：vscode PyCharm）；
- PyCharm python（3.12+）,用 uv 怎么安装 vllm（已经 0.12.0 了），PyTorch（已经 2.9 了），下载 4B 量化大模型，并运行；
...
- 1、用FastAPI+langGraph1.x+llamaIndex+fastMCP构建Agentic RAG系统；
- 2、要考虑公司分部门，部门分经理、主管、普通职员，部门内容可对内、对外的情况；
- 3、RAG评估也要有；
- 4、原始资料有复杂pdf、简单pdf、word、api、数据库等；
- 5、要考虑到数据更新，组织架构调整；
