...
ps:当服务端大量处于TIME_WAIT状态时意味着服务端正在经历大并发

- 半链接池（队列）：类似线程池，用于保存半链接的socket对象
  1. 半链接池：是固定的大小， 占满后，客户端请求收不到；
  2. 半链接：服务器主动向客户端发起连接，但客户端没有应答，则服务器会主动关闭半链接
  3. 半链接对象：保存了客户端的socket对象，等待服务器应答

服务器的内存给应用程序划一块内存（A），在这块内存（A）中再划出一块做半连接池；  
洪水攻击：大量请求，但是服务器没有处理，导致服务器内存溢出，导致服务器崩溃；
普通个人发起洪水攻击：在传输过程中，数据包会被运营商 负载均衡，转路由，七拐八拐，最后到目标服务器会分批次到达；
socket抽象层：封装应用层以外的所有层；
