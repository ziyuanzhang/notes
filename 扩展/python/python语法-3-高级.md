...
### 二十三、 selectors模块 -- 底层轮训【不做了解，直接用asyncio】
...
- 五种IO Model
  1. 阻塞IO(blocking IO)：特点就是在IO执行的两个阶段（等待数据和拷贝数据两个阶段）都被block了。
  2. 非阻塞IO(non-blocking IO)：在非阻塞式IO中，用户进程其实是需要不断的主动询问kernel数据准备好了没有【❌绝不被推荐】。
  3. 多路复用IO(multiplexing IO)/事件驱动IO(event driven IO):
