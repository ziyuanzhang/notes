...
1. 洪水攻击（SYN Flood）:
   ...
   - 防御手段：
     ...
     2. 增大队列
     3. 限速 / 防火墙
2. socket抽象层：封装应用层以外的所有层；
   - 👉 socket = 操作系统提供的 API，用来操作 TCP/IP 协议栈
3. 程序结束后：socket会释放，但不是“立刻完全消失”

   ```code
   进程退出
   → OS 自动 close 所有 socket
   → TCP 开始四次挥手
   → 进入 TIME_WAIT（可能持续 1~4 分钟）
...
- 半链接池（队列）：类似线程池，用于保存半链接的socket对象
  1. 半链接池：是固定的大小， 占满后，客户端请求收不到；

服务器的内存给应用程序划一块内存（A），在这块内存（A）中再划出一块做半连接池；

普通个人发起洪水攻击：在传输过程中，数据包会被运营商 负载均衡，转路由，七拐八拐，最后到目标服务器会分批次到达；

程序一旦结束：系统的套接字是否回收
应用程序资源：python自动回收； 系统资源：手动回收；

为什么要有socket？
socket是c/s架构独有的？B/S有吗？
socket与 浏览器 关系？
每次发消息secke都会3次握手吗？还是握手后一直连着？如果有很多客户端来连接，会不会撑爆连接（处理不过来）？
