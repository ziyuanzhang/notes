...
- ASCII码表：0-127用8位表示（实际7位就够了，多给1位为了扩展）；

- 打字到看到字过程：在文本编辑器输入“上”字，参照“字符编码表”，将“上”字转换成对应的二进制数存到内存中;文本编辑器将二进制数从内存中读取，再参照“字符编码表”转换成对应的字符，将字符输出到屏幕。
  1. 上--》翻译--》010101（二进制）-->存到内存中；
  2. 文本编辑器--》从内存读取010101--》翻译（参考同一张字符编码表）--》上--》显示到屏幕中；

- 软件运行过程产生的数据：先存在内存中；
  1. 现在内存默认编码：utf-8
  2. `ctrl+s` 硬盘保存数据时选择编码：可能乱码（看编码兼容）；
  3. 取的时候选择不同的编码，也可能乱码；

- py解释器执行文件流程：
  1. 启动py解释器【文本编辑器】；
  2. py解释器将文件（程序）从“硬盘”加载到“内存”【加载文件】；
  3. 开始识别py语法；

```python
# coding=utf-8  #这不是注释；是告诉解释器，这个文件使用utf-8编码 (不用默认编码)
print('上')
```

ASCII码是首个字符编码表，后续所有的字符编码表都兼容ASCII码（即支持英文）；
先用默认编码格式开始读，碰到首行的编码格式`coding=utf-8`，就会用这个编码格式去读文件；
