...
2. await（协程）
   ...
   - 特点：
     1. await 是协程的调度点（suspension point）。await 是协程主动让出 CPU 的地方。
...
6. python/fastApi中的 事件循环 与 线程关系
   - 线程是 操作系统调度 CPU 的基本单位。一个 Python 进程可以拥有多个线程：
   - Event Loop 是一个"程序（调度器）"，一个 Python 对象。它只是运行在线程里的一个调度器（Scheduler）对象。它不是线程；
     它负责：
     - 创建、管理 Task
     - 调度协程（Coroutine）
     - 等待 I/O 完成
   ...
   # ====== 同步代码 =====================================
     @app.get("/")
     def download():
     # HTTP请求 --> Main Thread --> Event Loop --> TreadPool --> Worker Thread --> 执行 download() --> 返回结果 给--> Event Loop Response
     # 这样：同步阻塞代码,不会阻塞 Event Loop。

   # ====== 异步代码 =====================================
     @app.get("/")
     # asyncHTTP请求 --> Main Thread --> Event Loop --> 协程 --> await数据库 -->Event Loop 去处理其它请求 --> 数据库完成 --> 恢复协程 --> 返回Response
   ```
