# python相关扩展

❗ 👉👇👈☝️ 🔥 1️⃣ ❌ ✅ ⚠️ 📌 🚀 👍 🚫 🆚 🎯 💡 🌐 ⏱️ 🔒 📊 🤔 📦 🔑 🟢 🔵 🟡

⚠️ 1. xx是什么？2. 为什么有它？3. 怎么用？

纠错、补充、优化、建立层级关系、总结

加密、签名、编码

- 字典(dict)、json、pydantic的模型：
  1. "JSON 字符串": 是用于传输的纯文本格式（是字符串）;
  2. "JSON 对象”": 在 Python 中通常映射为 dict字典 (就是"JSON 字符串"解析后得到的 "Python dict");

- 字典(dict)、dataclass、pydantic2
  1. dict：最基础的数据结构，没有字段约束。
  2. dataclass：官方的轻量级数据类，自动生成样板代码，但不负责验证数据。
  3. Pydantic BaseModel：面向数据验证和序列化的数据模型，特别适合 Web API（如 FastAPI）。
  4. Pydantic = dataclass + 类型校验 + 数据转换 + 序列化

  ```code
     JSON字符串
         │
     json.loads()
         │
         ▼
     Python dict
           │
           ├────────► User(**data) --► dataclass
           │
           └────────► User.model_validate(data) --► Pydantic Model
  ```

- Schema 与 Pydantic2
  1. Schema（模式）本质上是对数据结构的蓝图或契约，
  2. Pydantic 2 是 Python 生态中用于定义、验证和生成 Schema 的最强大工具之一；

- uv 与 uvicorn
  1. uv 是负责管理项目依赖和环境的“包管理器”，
  2. uvicorn 是负责运行 Web 应用的“服务器”。

## 管理工具

| 排名    | 工具             | 定位                           | 评价              | 镜像体积   | 冷启动时间    | 构建时间     |
| ------- | ---------------- | ------------------------------ | ----------------- | ---------- | ------------- | ------------ |
| 🥇 NO.1 | uv               | 全家桶（环境 + 包管理 + 构建） | 🚀 未来标准       | 100∼180MB  |    1.2∼2.8s   |  <45s        |
| 🥈 NO.2 | venv + pip-tools | 稳定组合                       | 🧱 企业保守首选   |   90∼150MB |     1.8∼3.5s  |    <90s      |
| 🥉 NO.3 | poetry           | 项目管理                       | ⚠️ 逐渐被 uv 替代 | 450∼800MB  |    8∼15s      |     3∼7min   |
| ❌ NO.4 | pipenv           | 已过气                         | 基本淘汰          | 600MB+     |     12s+      |      5min+   |
| 💀 NO.5 | conda            | 科学计算                       | ❗仅限特定领域    | 1.2∼3GB    |    25∼60s     |      8∼20min |

- 👉 一句话：uv = Python 世界的 npm + cargo；
- 👉 conda 是“重型武器”，不是日常工具（AI / 数据科学）；
  - conda 解决的是 系统级依赖（C库）；
  - pip/uv 解决不了 CUDA / MKL / BLAS；
- Ruff 正在成为 Python 的 eslint

| 产品名称 | 核心功能         | 替代的传统工具                             | 核心优势                                                                       |
| :------- | :--------------- | :----------------------------------------- | :----------------------------------------------------------------------------- |
| uv       | 包管理与环境管理 | `pip`, `pip-tools`, `Poetry`, `virtualenv` | 极速解析依赖（毫秒级），自动管理虚拟环境，不仅是安装器，更是项目构建工具。     |
| Ruff     | 代码检查与格式化 | `Flake8`, `Black`, `isort`, `pydocstyle`   | 速度比传统工具快 10-100 倍，内置 800+ 条规则，支持自动修复，是目前的行业标准。 |
| ty       | 静态类型检查     | `mypy`, `pyright`                          | Astral 较新的产品，专注于在大型代码库中进行高性能的全局类型安全扫描。          |

| 类别     | 传统/内置工具                   | 推荐现代工具 | 核心优势                   |
| :------- | :------------------------------ | :----------- | :------------------------- |
| 代码质量 | Flake8, Black, isort,pydocstyle | Ruff         | 极速，多合一               |
| 包管理   | pip, virtualenv                 | uv           | 毫秒级安装，统一环境       |
| 命令行   | argparse, click                 | Typer        | 基于类型提示，开发极快     |
| 终端显示 | print                           | Rich         | 漂亮的表格、颜色、Markdown |
| 数据处理 | Pandas                          | Polars       | Rust 内核，更快更省内存    |
| HTTP请求 | requests                        | httpx        | 支持异步，现代标准         |
| 数据校验 | 手动 if/else                    | Pydantic     | 自动类型检查，安全可靠     |

### 总结建议

如果你是一个追求现代化开发体验的 Python 开发者，我推荐的“2026 黄金组合”是：

- ✅ 必选
  - 包管理：uv
  - 代码质量：Ruff

- ✅ 强烈推荐
  - 类型检查：pyright
  - CLI：Typer
  - 终端输出：Rich
  - 数据模型：Pydantic v2
  - HTTP：httpx

- ✅ 数据场景
  - 新项目：Polars
  - 旧项目：Pandas

## uv 安装与使用

1. 安装
   - macOS / Linux: `curl -LsSf https://astral.sh/uv/install.sh | sh`
   - Windows (PowerShell): `powershell -ExecutionPolicy ByPass -c "irm https://astral.sh/uv/install.ps1 | iex"`
   - 备选安装方式
     - macOS (Homebrew): `brew install uv`
     - Windows (Winget): `winget install uv`
     - 通过 pipx 隔离安装: `pipx install uv`

2. 验证安装：`uv --version`
3. 初始化新项目：`uv init my_project`; `cd my_project`
4. 管理 Python 版本:
   - 安装指定的 Python 版本：`uv python install 3.12`
   - 锁定当前项目的 Python 版本：`uv python pin 3.12`（会生成或更新 .python-version 文件）

5. 管理依赖包
   - 添加依赖：使用 uv add 安装包，并自动将其记录到 pyproject.toml 中 `uv add requests`。
   - 添加开发依赖（如测试工具）：`uv add --dev pytest`

6. 同步项目环境: `uv sync`
   - 当你在团队协作中拉取了包含 pyproject.toml 和 uv.lock 的项目时，只需一键同步，uv 会自动创建虚拟环境并安装所有依赖：

7. 运行代码（最省心的命令）: `uv run python main.py`

- 使用 uv run 是最推荐的运行方式。它会自动在项目的虚拟环境中执行代码，完全不需要你手动去 source activate 激活环境：

## 安装并初始化项目

1. 初始化项目: `uv init my_project` ；`cd my_project`;
2. 创建虚拟环境并激活：
   - `uv venv`;
   - Linux/macOS: `source .venv/bin/activate`
   - Windows (PowerShell/CMD): `.venv\Scripts\activate`
3. 添加”.python-version “文件，在src下写代码；
4. git:
   - `git add uv.lock`
   - .gitignore（最小但正确）

   ```gitignore
     .venv/
     __pycache__/
     .mypy_cache/
     .pytest_cache/
     .ruff_cache/
     .env
   ```

5. 安装开发依赖:`uv add --dev pytest mypy ruff`
   在 pyproject.toml 中添加：

   ```python
      [tool.mypy]
      python_version = "3.11"
      strict = true

      # 项目结构
      packages = ["my_project"]
      mypy_path = ["src"]

      # 常见妥协
      ignore_missing_imports = true
      warn_unused_ignores = true
      warn_return_any = true
      warn_unreachable = true

      [tool.ruff]  # 格式 + lint
      target-version = "py311"
      line-length = 88

      [tool.ruff.lint] # 格式 + lint
      select = ["E", "F", "I", "B", "UP"]
      ignore = ["E501"]
   ```

   - 执行格式检查和格式化：`uv run ruff check .`；`uv run ruff format .`；
   - 执行类型检查：`uv run mypy src`；

6. 同步下载pyproject.toml中的依赖：`uv sync`
   👉 uv + mypy + ruff + src 布局
   👉 FastAPI + uv + mypy + ruff 标准骨架
   👉 LangGraph / LlamaIndex 项目如何用 uv 管理多子模块

## PyCharm 插件

file--> settings--> plugin (安装完重启 pycharm)

1. 代码统计（Statistics）: 按完在pyCharm左下角，点击-->刷新（refresh）查看；
2. 错误检查（Inspections/ Code Inspection）: 内置；
3. 错误修复（Quick Fix）: 内置；
4. Rainbow Brackets (彩虹括号)：
5. SonarLint (代码体检)：比 PyCharm 自带的检查更严格；
