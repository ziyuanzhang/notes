...
## 十、ORM (Object Relational Mapping): 对象关系映射 -- 方便数据库迁移
...
| `NOT NULL`                    | ✅ 标准SQL       | 字段不能为空           | `name VARCHAR(50) NOT NULL`                       | 用户名、密码       | 真实开发大量使用，减少 `NULL` 带来的逻辑复杂度。                    |
| `UNIQUE`                      | ✅ 标准SQL       | 字段值不能重复         | `email VARCHAR(100) UNIQUE`                       | 邮箱、手机号       | 自动创建唯一索引；一个表可以有多个 `UNIQUE`。                       |
| `DEFAULT`                     | ✅ 标准SQL       | 设置默认值             | `status INT DEFAULT 1`                            | 状态字段、时间字段 | 插入数据时未传值，则自动使用默认值。                                |
| `FOREIGN KEY`                 | ✅ 标准SQL       | 建立表关系             | `FOREIGN KEY(user_id) REFERENCES users(id)`       | 用户-订单关系      | 保证引用数据存在；大型互联网项目很多使用“逻辑外键”代替。            |
