...
- Collection - `单列集合【祖宗接口】`：每个元素只包含一个值（数据）；
  ...
  1. `List<E>`:【接口】添加的数据是 有序（指添加先后顺序）、可重复、有索引；
     ...
     - `LinkedList<E>`:【实现类】有序、可重复、有索引；底层基于“链表”存储数据；
...
- ArrayList（基于数组） 与 LinkedList（基于链表）：
  1. 数组：连续存储; 寻址较快,增删较慢（位置向后移动--扩容）；
  2. 数组查询：通过地址和索引定位，查询任意数据耗时相同；
  3. 零基索引(索引从 0 开始)：`元素地址 = 基址(物理地址) + 偏移量(单位制 * 索引)`();
  4. 一基索引(索引从 1 开始)：`元素地址 = 基址(物理地址) + (偏移量 - 1)`，多一次减法操作
  5. `偏移量 = 单位制 * 索引`
