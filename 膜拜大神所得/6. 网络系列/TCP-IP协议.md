...
3. 网络层 ---->IP 协议（IP 地址-->IP 地址）；提供 from(从哪来)IP，to（到哪去）IP
   ...
   - IP 数据：把传输层数据切割、封装成 IP 包（Packet）

   `网关`：网关 = 默认出口（下一跳）；通常是路由器的一个接口IP

4. 链路层 ---->多种底层网络协议，例：Ethernet（以太网）,wifi(设备-->设备)；提供 from(原来)mac 地址，to（到哪去）mac 地址
...
2. 子网掩码（Subnet Mask）: 用来判断：谁跟我是一个局域网
...
3. 网关（Gateway）: 就是“出门的路口”
   - 例：192.168.1.1（通常是路由器）
   - 👉 作用：当目标不在局域网 → 发给网关

4. 网卡（NIC）与 MAC地址（MAC Address）
   - 电脑的“网口”
