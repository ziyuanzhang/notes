...
- 基本类型（原始数据类型） ：String、Number、Boolean、Null（空）、Undefined（未定义）、Symbol 、Bigint
...
1. 栈（stack）：存原始数据类型；堆（heap）：存引用数据类型（对象、数组和函数）
...
2. 在操作系统中，内存被分为栈区和堆区：
   ...
   - 堆区内存：一般由开发者分配释放，若开发者不释放，程序结束时可
     能由垃圾回收机制回收。
3. 栈（stack）：先进后出【向中“桶”存取物品】；堆（heap）：【书架中的书，知道书名就可以拿到书，可以无序】
