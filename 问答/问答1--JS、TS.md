...
1. 基本类型的数据（数值、字符串、布尔值）：变量名和值都保存在“执行上下文-->词法环境-->环境记录”中；
2. 引用类型的数据（主要是对象和数组）：
   - 变量名：存在“执行上下文-->词法环境-->环境记录”中；保存一个指针，指向‘值’所在的‘堆’；
   - 值：存在“堆”中；
...
1. 存储位置不同（‘执行上下文-->词法环境-->环境记录’和堆）
2. 访问机制不同（按值，引用访问）
3. 变量赋值不同（值的副本，引用）
4. 参数传递不同（传值，引用）

- 引用：不是内存地址，是变量名与‘值的内存地址’的映射关系；

## JavaScript 的内存结构（以 V8 为例）

- JS 引擎（如 V8）通常包含：
  1. 调用栈（Call Stack）
  2. 堆（Heap）
  3. 执行上下文（Execution Context）
  4. 词法环境（Lexical Environment）👉 词法环境是执行上下文的一部分。

`let x=10` JS运行时：

1. 创建“执行上下文”，包含：可变环境、词法环境、this绑定；
2. x 会被存储在：词法环境--》环境记录，本质是一个“变量映射表”；
   - 注意：这不是“栈区的一块连续内存”，而是引擎内部的环境结构。
3. 值：基本（原始）数据类型：通常会直接存在栈（或栈附近的快速内存）
4. 值：引用类型（对象）：`let obj={a:1}` 对象在堆中创建，obj存对象地址；

- JavaScript 是“执行上下文 + 词法环境模型”，不是“栈变量模型”。
- “变量存在于环境记录” 意思是：它是一个‘运行时结构’，不是‘物理连续栈内存’，不是‘编译期固定偏移’，变量名在运行时依然存在；
- 变量 → 环境记录
  对象 → 堆
  原始值（基本数据类型的值） → 通常直接存储（存在‘环境记录）
...
1. typeof
   ...
   - typeof (object、Function、Array、Date、Number、String、Boolean)-->结果是:function
...
![async-defer](./img/async-defer.jpg)
...
```js
// 1、structuredClone
structuredClone; //let newObj = structuredClone(obj)
// 2、
...
- 防抖（debounce）：👉 只要你还在动，我就不执行

  你先别急，等你停下来我再干活;
  1. 按钮提交
  2. input 输入

- 节流（throttle）：👉 你随便动，我按节奏执行

  你再怎么疯狂点，我最多每隔一段时间干一次;
  1. 监控浏览器 resize；
  2. 元素拖拽
...
```js
let xhr = new XMLHttpRequest(); //post请求一定要添加请求头才行不然会报错
xhr.setRequestHeader("Content-type", "application/x-www-form-urlencoded"); //设置请求的类型及url
...
let json = {}; //遍历 str 拆解其中的每一个字符将其某个字符的值及出现的个数拿出来作为 json 的 kv
